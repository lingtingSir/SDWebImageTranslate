 ...
 * file that was distributed with this source code.
 ...
 SDWebImageManager管理着缓存SDImageCache和下载SDWebImageDownloader类的对象。我们在这个类可以得到关于下载和缓存的相关状态。本类有12个关于管理的SDWebImageOptions操作类型，与SDWebImageDownloader的 SDWebImageDownloaderOptions操作类型相似。
 */
 ...
 *  @param image     预存好的图片
 *  @param error     错误信息
 *  @param cacheType 使用了 SDImageCacheType 选项<
 *  @param imageURL  预存的Url
 */
...
/**
 *  控制该图像时在高速缓存中未找到该图像应被下载。
 *
 *  @param imageManager 目前`SDWebImageManager`
 *  @param imageURL     图像的URL下载
 *
 *  @return 不返回任何防止图像的下载的高速缓存未命中。如果不执行，YES是隐含的。
 */
...
/**
 *  允许它已被下载，只是缓存在磁盘上和内存后，才立即转换图像。注：此方法是从全局队列叫以不阻塞主线程。
 *
 *  @param imageManager 目前`SDWebImageManager`
 *  @param image        图片转换图像
 *  @param imageURL     图像的URL转换
 *
 *  @return 转换后的图像对象。 注意，该方法在全局队列里面完成 为了不堵塞主线程。
 */
